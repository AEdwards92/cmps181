...
void Project::projectFields(const void *input, void *data,
...
RC NLJoin::getNextTuple(void *data) {
	...
	do {
		...
		ret = rightItr->getNextTuple(rightTuple);
...
INLJoin::INLJoin(Iterator *leftIn, IndexScan *rightIn,
	...
	if (isEnd != QE_EOF) {
		...
		rightItr->setIterator(lowKey, highKey, lowKeyInclusive,
				highKeyInclusive);
...
RC INLJoin::getNextTuple(void *data) {
	...
	do {
		...
		if (isSuccess == QE_EOF && leftHalf && op == NE_OP) {
			...
			setCondition(op, &lowKey, &highKey, lowKeyInclusive,
					highKeyInclusive);
			rightItr->setIterator(lowKey, highKey, lowKeyInclusive,
					highKeyInclusive);
