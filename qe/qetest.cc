...
int testCase_1() {
    ...
    cout <<"BEFORE BBBBBBB"<<endl;
	rc = populateLeftTable();
	if (rc != success) {
		return rc;
	}
    cout <<"BEFORE CCCCCCC"<<endl;
...
int testCase_7() {
	...
	*(int *) value.data = compVal;
	...
	// Create INLJoin
	INLJoin *inlJoin = new INLJoin(project, rightIn, cond, 10);
...
int testCase_9_Grad() {
	...
	*(int *) value.data = compVal;
    ...
    while (rightIn->getNextTuple(data) != QE_EOF) {
		...
		// Print right.A
		cout << "right.A " << *(int *) ((char *) data + offset) << " ";
		offset += sizeof(int);
        
		// Print right.B
		cout << "right.B " << *(float *) ((char *) data + offset) << " ";
		offset += sizeof(int);
        
		// Print right.C
		cout << "right.C " << *(int *) ((char *) data + offset) << endl;
		offset += sizeof(float);
        
        memset(data, 0, bufSize);
	}*/
    
	join = new INLJoin(project, rightIn, cond_j, 10);
