...
#define TEST_FN_POSTFIX(msg) { ++numPassed; cout << ' ' << numTests << ") OK: " << msg << endl; } \
                        else { cout << ' ' << numTests << ") FAIL: " << msg << "<" << err::errToString(rc) << ">" << endl; }
...
RC testMaxSizeRecords(FileHandle& fileHandle, int recordSizeDelta, int minRecordSize, bool insertForwards)
{
    ...
    const int maxRecordSize = PAGE_SIZE - sizeof(PageIndexEntry) - sizeof(PageIndex) - sizeof(unsigned) * recordDescriptor.size() - sizeof(unsigned) - sizeof(unsigned);
    ...
    if (ret == err::OK)
    {
        for (int size = minRecordSize, index = 0; size <= maxRecordSize; size += recordSizeDelta, ++index)
        {
            ...
            if (ret == err::OK && memcmp(buffersIn[index], buffersOut[index], size))
            {
                std::cout << "There was an error checking the big string[" << index << "], size=" << size << std::endl;
                ret = err::FILE_CORRUPT;
                break;
            }
        }
    }
...
void rbfmTest()
{
	...
	TEST_FN_EQ( err::OK, testSmallRecords2(handle1), "Testing inserting many tiny records in batches");
    ...
    TEST_FN_EQ( err::OK, testMaxSizeRecords(handle2, 1, 9 * PAGE_SIZE / 10, true), "Testing insertion of large records");
    ...
    TEST_FN_EQ( err::OK, testMaxSizeRecords(handle3, 257, sizeof(int) + sizeof(char), false), "Testing insertion of varying records");
    ...
    TEST_FN_EQ( err::OK, testMaxSizeRecords(handle4, 3000, sizeof(int) + sizeof(char), true), "Testing insertion of varying records");
    ...
    TEST_FN_EQ( err::OK, testRandomInsertion(handle4, 1, 1, 2, false), "Testing insertion of randomly sized tiny records");
    TEST_FN_EQ( err::OK, testRandomInsertion(handle4, 20, 1, 40, false), "Testing insertion of randomly sized medium records");
    TEST_FN_EQ( err::OK, testRandomInsertion(handle4, 66, 1, 80, false), "Testing insertion of randomly sized large records");
    TEST_FN_EQ( err::OK, testRandomInsertion(handle4, 333, 1, 160, false), "Testing insertion of randomly sized huge records");
	...
	TEST_FN_EQ( err::FILE_HANDLE_ALREADY_INITIALIZED, pfm->openFile("testFile0.db", handle1), "Open testFile0.db and store in already initialized file handle");
	...
	TEST_FN_EQ( 0, pfm->openFile("testFile4.db", handle4), "Open testFile4.db and store in handle4");
	TEST_FN_EQ( err::FILE_COULD_NOT_DELETE, pfm->destroyFile("testFile4.db"), "Delete testFile4.db with open handles");
	TEST_FN_EQ( 0, pfm->closeFile(handle1), "Close testFile1.db");
	TEST_FN_EQ( err::FILE_HANDLE_NOT_INITIALIZED, pfm->closeFile(handle1), "Close testFile0.db again");
	...
	TEST_FN_EQ( err::FILE_HANDLE_NOT_INITIALIZED, pfm->closeFile(handle4), "Close testFile4.db through uninitialized file handle");
	TEST_FN_EQ( 0, pfm->destroyFile("testFile4.db"), "Delete testFile4.db with open handles");

	// Test inserting and deleting records
	TEST_FN_EQ( 0, pfm->createFile("testFile4.db"), "Create testFile4.db");
	TEST_FN_EQ( 0, pfm->openFile("testFile4.db", handle4), "Open testFile4.db and store in handle4");
	TEST_FN_EQ( err::OK, testRandomInsertion(handle4, 20, 1, 40, true), "Testing insertion and deletion of records");
	TEST_FN_EQ( 0, pfm->closeFile(handle4), "Close handle4");
...
void pfmTest()
{
	...
	TEST_FN_EQ( err::OK, pfmKillProcTest2(), "Test that we can 'kill' the process after createFile");
	TEST_FN_EQ( err::OK, pfmKillProcTest3(), "Test that we can 'kill' the process after openFile");
	TEST_FN_EQ( err::OK, pfmKillProcTest4(), "Test that we can 'kill' the process after writing a page");
...
void fhTest()
{
    ...
    TEST_FN_EQ(err::FILE_ALREADY_EXISTS, pfm->createFile(fileName.c_str()), "Create file that already exists");
    ...
    TEST_FN_EQ(success, pfm->openFile(fileName.c_str(), fileHandle), "Open file");
    TEST_FN_EQ(err::FILE_HANDLE_ALREADY_INITIALIZED, pfm->openFile(fileName.c_str(), fileHandle), "Open file fails when same filehandle already pointing to an open file");
    ...
    TEST_FN_EQ(err::FILE_PAGE_NOT_FOUND, fileHandle.writePage(1, buffer), "Writing to non-existent page");
    TEST_FN_EQ(err::FILE_PAGE_NOT_FOUND, fileHandle.writePage(0, buffer), "Writing to non-existent page");
    ...
    TEST_FN_EQ(success, pfm->openFile(fileName.c_str(), fileHandle), "Open file");
    TEST_FN_EQ(err::FILE_PAGE_NOT_FOUND, fileHandle.readPage(1, buffer_copy), "Reading from non-existent page");
    ...
    TEST_FN_EQ(err::FILE_COULD_NOT_DELETE, pfm->destroyFile(fileName.c_str()), "Destroy attempt #1 when file is still open (two pins)");
    rc = pfm->closeFile(fileHandle2);
    assert(rc == success);
    TEST_FN_EQ(err::FILE_COULD_NOT_DELETE, pfm->destroyFile(fileName.c_str()), "Destroy attempt #2 when file is still open (one pin)");    
    TEST_FN_EQ(err::FILE_HANDLE_NOT_INITIALIZED, pfm->closeFile(fileHandle2), "Multiple close through the same handle");
    ...
    string dummy = "dummy"; 
    TEST_FN_EQ(err::FILE_COULD_NOT_DELETE, pfm->destroyFile(dummy.c_str()), "Destroy non-existent file");
...
RC rbfmTestReorganizePage(RecordBasedFileManager *rbfm, int skip)
{
    ...
    for (PageNum page = 1; page < fileHandle.getNumberOfPages(); page++)
    {
    	...
    	assert(rc == err::OK || rc == err::PAGE_CANNOT_BE_ORGANIZED);
    }
...
RC rbfmTestUpdateRecord(RecordBasedFileManager *rbfm, int skip)
{
	RC rc = err::OK;
    string fileName = "rbfmTestUpdateRecord_file";
...
RC rbfmTestReorganizeDeleteUpdate(RecordBasedFileManager *rbfm, int skip)
{
    ...
    for(int i = 0; i < numRecords; i++)
    {
        ...
        // Record the RIDs
        cout << "Just inserted RID: " << rid.pageNum << ", " << rid.slotNum << endl;
        rids.push_back(rid);
        sizes.push_back(offset);
    }
    ...
    {
    	cout << "Deleting " << rids[i].pageNum << "," << rids[i].slotNum << endl;
    	RC this_ret = rbfm->deleteRecord(fileHandle, recordDescriptor, rids[i]);
        cout << "Return code: " << err::errToString(this_ret) << ", errno: " << this_ret << endl; 
    }
    ...
    for (int i = 2; i < numRecords; i += skip)
    {
        ...
        // Update the record
    	cout << "Shrinking " << rids[i].pageNum << "," << rids[i].slotNum << endl;
        rc = rbfm->updateRecord(fileHandle, recordDescriptor, record, rids[i]);
        if (rc != err::RECORD_DELETED)
        {
            assert(rc == success);
